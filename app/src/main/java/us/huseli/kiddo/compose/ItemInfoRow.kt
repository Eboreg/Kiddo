...
import us.huseli.retaintheme.extensions.clickableIfNotNull
...
fun ItemInfoRow(label: String, text: String, onClick: ((String) -> Unit)? = null) {
    ...
    ItemInfoRow(label = label) {
        Text(text, modifier = Modifier.clickableIfNotNull(onClick, text), style = textStyle, textAlign = TextAlign.End)
    }
}
...
@Composable
fun ItemInfoRow(label: String, content: @Composable RowScope.() -> Unit) {
    Row(
